...
filter_project_diagnostics() {
    ...
    project_file_regex="^${escaped_workspace_dir}(app|core|ui|tests)/.*:[0-9]+:[0-9]+: (warning|error):"
...
run_single_file() {
    ...
    escaped_workspace_dir="$(escape_regex "${workspace_dir}/")"
    header_filter="^${escaped_workspace_dir}(app|core|ui|tests)/"
...
make_tidy_compile_database() {
    ...
    temp_dir="$(mktemp -d "${TMPDIR:-/tmp}/qt-clang-tidy-db.XXXXXX")"
